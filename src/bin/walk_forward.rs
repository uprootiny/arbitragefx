...
fn main() {
    ...
    let csv_path = args.get(1).map(|s| s.as_str()).unwrap_or("data/btc_real_1h.csv");
    ...
    println!("Windows: {}, Train fraction: {:.0}%", num_windows, train_fraction * 100.0);
    println!();
    ...
    println!("Comparisons: {} ({} strategies x {} windows)",
        result.num_comparisons, result.num_strategies, result.num_windows);
    println!("Correction: {} (alpha = {})", result.correction_method, result.alpha);
    println!();

    println!("{:<14} {:>10} {:>10} {:>8} {:>6}/{:<6} {:>8} {:>10}",
        "Strategy", "Train PnL", "Test PnL", "Overfit", "Win", "Total", "p-value", "Survives?");
    println!("{}", "-".repeat(80));

    for s in &result.summaries {
        println!("{:<14} {:>10.4} {:>10.4} {:>8.2} {:>6}/{:<6} {:>8.4} {:>10}",
            s.id,
            s.train_mean_pnl,
            s.test_mean_pnl,
            s.overfit_ratio,
    ...
    let survivors: Vec<_> = result.summaries.iter()
        .filter(|s| s.survives_correction)
        .collect();

    println!();
    if survivors.is_empty() {
        println!("No strategies survive Bonferroni correction at alpha={}.", result.alpha);
        ...
        for s in &survivors {
            println!("  {} (test mean PnL: {:.4}, overfit ratio: {:.2})",
